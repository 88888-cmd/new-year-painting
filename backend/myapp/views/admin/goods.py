...
from myapp.models import GoodsCategory, Goods, GoodsSku, GoodsComment, FreightTemp, MediaFile
...
def get_list(request):
    result = [item.to_dict() for item in Goods.objects.all()]
...
def add(request):
    ...
    freight_temp_id = int(body.get('freight_temp_id', 0))
    skus = body.get('skus', [])
    ...
    try:
        with transaction.atomic():
            ...
            for sku_data in skus:
                ...
                price = check_amount(sku_data.get('price', '0'))
                line_price = check_amount(sku_data.get('line_price', '0'))

                sku_instances.append(GoodsSku(
                    goods_id=goods.id,
                    sku_name=sku_name,
                    price=price,
            ...
            GoodsSku.objects.bulk_create(sku_instances)
...
def edit(request):
    ...
    detail_url = body.get('detail_url', '')
    freight_type = int(body.get('freight_type', 0))
    freight_price = check_amount(body.get('freight_price', 0))
    ...
    try:
        with transaction.atomic():
            ...
            GoodsSku.objects.filter(goods_id=goods_id).delete()
            sku_instances = []
            for sku_data in skus:
                sku_name = sku_data.get('sku_name', '')
                price = check_amount(sku_data.get('price', '0'))
...
def delete(request):
    ...
    try:
        Goods.objects.get(id=goods_id, is_delete=0)
    except Goods.DoesNotExist:
        return APIResponse(code=1, msg='数据不存在')

    Goods.objects.filter(id=goods_id).update(is_delete=1)
