<template>
  <div>
    <el-card class="form-container" shadow="never">
      <aside style="margin-bottom: 20px">编辑积分任务</aside>
      <el-form label-width="120px" size="medium">
        ...
        <el-form-item label="指定年画">
          ...
          <el-input v-model="selectedPaintingName" readonly :placeholder="isPaintingLimited ? '请选择年画' : '已选择不限（所有年画）'"
            style="width: 100%; margin-top: 10px" :disabled="!isPaintingLimited">
            <template #append>
...
<script>
import { detail, edit } from '@/api/pointsTask';
...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      try {
        const paintingListResult = await getPaintingList();
        this.paintings = paintingListResult.data;
        ...
        this.selectedPaintingId = this.form.painting_id;
        this.isPaintingLimited = this.form.painting_id !== 0;

        if (this.form.painting_id !== 0) {
          const matchedPainting = this.paintings.find(p => p.id === this.form.painting_id);
          this.selectedPaintingName = matchedPainting ? matchedPainting.name : '';
        }
    ...
    clickSubmit() {
      ...
      edit({ task_id: this.$route.params.id, ...this.form })
        .then((response) => {
          that.$message({
            message: "编辑成功",
            type: "success",
