        ...
        <div>
          <el-table style="width: 100%" :data="form.skus" border>
            ...
            <el-table-column label="积分" align="center">
              <template slot-scope="scope">
                <el-input-number size="small" v-model="scope.row.point_num" :precision="0" :step="1" :min="0"
                  :max="9999999999" style="width: 100%"></el-input-number>
              </template>
            </el-table-column>
...
import { getList as getCategoryList } from "@/api/pointsGoodsCategory";
import { detail, edit } from "@/api/pointsGoods";
  ...
  methods: {
    ...
    clickSubmit() {
      ...
      for (const item of skus) {
        ...
        if (typeof item.point_num != "number") {
          this.$message.error("请输入规格积分");
          return;
        }
      }
      ...
      edit({ goods_id: this.$route.params.id, ...this.form })
        .then((response) => {
          ...
          this.$router.push({ path: "/pointsGoods/list" });
        })
        .finally(() => {
          this.disabled = false;
        });
